...
def detetar_e_anotar(texto: str,
                     termos_br_pt: dict) -> tuple:  # Define a função que recebe um texto e um dicionário BR→PT, devolvendo o texto anotado e o número de anotações.
    """
    ...
    :return: Tupla (texto anotado com sugestões, número de anotações)
    ...
    chaves_ordenadas = sorted(termos_br_pt.keys(), key=len,
                              reverse=True)  # Cria lista de chaves do dicionário, ordenada da mais longa para a mais curta.

    texto_anotado = texto  # Variável que guardará o texto já com anotações; começa igual ao texto original.
    total_anotacoes = 0  # Contador para o número de anotações realizadas
    ...
    for br_expr in chaves_ordenadas:  # Percorre cada expressão brasileira da lista ordenada.
        ...
        pattern = re.compile(rf"\b{re.escape(br_expr)}\b",
                             re.IGNORECASE)  # Compila o padrão regex com \b e flag IGNORECASE.

        # Conta ocorrências antes da anotação
        ocorrencias = len(pattern.findall(texto_anotado))
        total_anotacoes += ocorrencias
    ...
    return texto_anotado, total_anotacoes  # Devolve o texto final com anotações e o número de anotações aplicadas.
